...
def surface_temp(h_i, h_s, day, alb_surf, limit_temp=temp_lim):
                ...
                [
                    ...
                    np.roots(
                        [
                            ...
                            0,
                            0,
                            solar_flux(day) * (1 - alb_wat) + non_solar_flux(day),
                        ]
                    ).real[3],
                ]
            )
    ...
    def net_surf_flux(h_i, h_s, day, T_su, alb_surf):
    ...
    nsf = net_surf_flux(h_i, h_s, day, T_su, alb_surf)
...
def ice_thick(
    ...
    T_su, efm = surface_temp(h_i0, h_s0, 1, alb_surf, limit_temp=temp_lim)
    T_su_ar[0] = T_su
    ...
    alb_sur_ar[0] = 0.77  # initial value stored of the default surface albedo.
    ...
    for day in range(1, integration_range):
        ...
        if ice_cover == False:
            ...
            T_su, efm = surface_temp(
                h_i[day - 1], h_s[day - 1], day, limit_temp=temp_lim, alb_surf=alb_wat
            )
            T_su_ar[day] = T_su
            if T_w >= T_bo:
...
def ctrl_sim():
    ...
    plt.plot(time_range, alb_sur_ar, label="alb_sur", linewidth=0.4)
    plt.plot(time_range, h_snow, label="h_snow", linewidth=0.4)
    plt.title(
